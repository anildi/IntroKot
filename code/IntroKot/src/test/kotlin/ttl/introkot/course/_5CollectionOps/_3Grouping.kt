...
/**
 * Another Groupingby.  Here we are going to get a map of zip code
 * and person with the most phone numbers.  If two persons have the
 * same number of phone numbers, the later one gets into the map.
 */
fun groupingByZipCodeAndPersonWithMostPhoneNumbers(): Map<String, Person> {

    val map2 = persons.groupingBy {
        it.address.zipCode
    }.reduce{key, currPerson, newPerson ->
        if(newPerson.phoneNumbers.size >= currPerson.phoneNumbers.size)
            newPerson
        else
            currPerson
    }
 ...
 * zipCode by the sum of the points for all
...
fun groupingByAggregation(): Map<String, Int?> {
    val map: Map<String, Int?> = persons.groupingBy {
        ...
        if (!isFirst) { //if(accumulator != null) {
            accumulator?.plus(currPerson.points)
        } else {
            currPerson.points
        }
    }
...
class TestGrouping {
    ...
    @Test
    fun testGroupingByZipCodeAndPersonWithMostPhoneNumbers() {
        val m3 = groupingByZipCodeAndPersonWithMostPhoneNumbers();
        for((key, value) in m3) {
            println("$key: $value")
        }
    }
