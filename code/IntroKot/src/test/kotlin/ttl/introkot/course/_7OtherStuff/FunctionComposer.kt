...
//    sqThenDouble(2).also {println("Final: $it")}
    ...
    fun testPredicateAndOr() {
        ...
        val q = nameMatch and lenMatch or vMatch //and {str: String -> str.length > 4}

        q("voo").also {
            println("from q: $it")
            assertFalse(it)
        }
        val filteredToo = list.filter(q).also { println("List: $it") }
        assertEquals(3, filteredToo.size)
